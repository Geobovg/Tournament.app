...
export default async function ManagerCareerPage() {
  ...
  const [career, manager, challenges, matches] = await Promise.all([getCareerProfile(user.id), getManagerCareer(user.id), getCareerChallenges(user.id), listManagerMatchHistory(user.id)]);
