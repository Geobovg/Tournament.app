...
function TournamentCard({
  ...
  return (
    <li className="flex items-center gap-2">
      ...
      {isOwner ? <TournamentSettings tournamentId={tournament.id} tournamentName={tournament.name} inviteToken={tournament.invite_token} inviteCode={tournament.invite_code} members={members.filter((member) => member.user_id !== currentUserId)} friends={friends.filter((friend) => !members.some((member) => member.user_id === friend.id))} registrationOpen={tournament.status === "registration"} /> : null}
    </li>
  );
}
