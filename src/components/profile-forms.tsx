  ...
  return (
    <div className="grid gap-6">
      <section className={`${cardClass} grid gap-4`}>
        ...
        {avatarUrl ? <Image src={avatarUrl} alt="Profilbilde" width={80} height={80} className="h-20 w-20 rounded-full object-cover" /> : <div className="flex h-20 w-20 items-center justify-center rounded-full bg-accent-soft text-2xl font-semibold text-accent">{username.slice(0, 1).toUpperCase()}</div>}
        <form action={avatarAction} className="grid gap-3"><input name="avatar" type="file" accept="image/png,image/jpeg,image/webp" required />{avatarState.error ? <p className="text-danger">{avatarState.error}</p> : null}{avatarState.message ? <p className="text-success">{avatarState.message}</p> : null}<button className={secondaryButtonClass} disabled={uploading}>{uploading ? "Laster opp…" : "Legg til eller endre profilbilde"}</button></form>
      </section>
      <section className={`${cardClass} grid gap-4`}>
