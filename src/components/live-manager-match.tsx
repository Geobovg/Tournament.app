...
import { getManagerKickoff, getManagerSubstitutions, scoreAtMinute, simulateManagerTimeline, teamAfterSubstitutions, type ManagerTactics } from "@/lib/manager-match";
...
export function LiveManagerMatch({ match, userId }: { match: { id: string; status: string; started_at: string | null; home_score: number; away_score: number; events: unknown; tactics: ManagerTactics | null }; userId: string }) {
  ...
  const goals = simulateManagerTimeline(match.id, match.events, match.tactics);
  ...
  const visibleGoals = goals.filter((goal) => goal.minute <= virtualMinute);
  ...
  return <section className={`${cardClass} grid gap-5 text-center`}>
    ...
    {match.status === "live" && fullTime ? <p className="text-sm text-muted">Sluttresultatet lagres automatisk…</p> : null}
    <form ref={finishFormRef} action={finishAction}><input type="hidden" name="match_id" value={match.id} /></form>
    {state.error ? <p className="text-sm text-danger">{state.error}</p> : state.ok ? <p className="text-sm text-success">Resultatet, V/U/T og belønningen er lagret.</p> : null}
  </section>;
}
