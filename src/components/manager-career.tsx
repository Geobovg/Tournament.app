...
function SquadCard({ card, position, active, onPointerDown, onPointerUp, onClick, selected }: { card: ManagerCard; position: string; active: boolean; onPointerDown: (event: React.PointerEvent<HTMLButtonElement>) => void; onPointerUp: (event: React.PointerEvent<HTMLButtonElement>) => void; onClick: () => void; selected: boolean }) {
...
function Squad({ cards, lineup }: { cards: ManagerCard[]; lineup: ManagerLineup | null }) {
  ...
  return <section className={`${cardClass} grid gap-5 overflow-hidden`}><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold tracking-[.22em] text-cyan-300">SQUAD</p><h2 className="mt-1 text-2xl font-black">Troppen min</h2><p className="mt-1 text-sm text-muted">Dra kort for å bytte spillere. Du trenger 11 i elleveren og 7 på benken før laget kan lagres.</p></div><span className="rounded-lg bg-accent-soft px-3 py-2 text-sm text-accent">{cards.length} / {squadCapacity}</span></div>
    <form action={action} className="grid gap-4"><input type="hidden" name="formation" value={formation} />{starters.map((id) => <input key={`starter-${id}`} type="hidden" name="starter_ids" value={id} />)}{bench.map((id) => <input key={`bench-${id}`} type="hidden" name="bench_ids" value={id} />)}
      ...
      <div><div className="mb-2 flex items-center justify-between"><h3 className="text-sm font-black tracking-wide">BENK</h3><span className="text-xs text-muted">Dra spillere hit for å bytte</span></div><div className="flex flex-wrap gap-3">{bench.map((id) => { const card = cardById.get(id); return card ? <div key={id} className={benchCardClass}><SquadCard card={card} position={card.position} active={draggedId === id} selected={selectedId === id} onPointerDown={pointerDown(id)} onPointerUp={pointerUp()} onClick={() => setSelectedId(id)} /></div> : null; })}</div></div>
      <div><div className="mb-2 flex items-center justify-between"><h3 className="text-sm font-black tracking-wide">RESERVER</h3><span className="text-xs text-muted">{reserves.length} tilgjengelige</span></div>{reserves.length ? <div className="flex flex-wrap gap-3">{reserves.map((card) => <div key={card.id} className={benchCardClass}><SquadCard card={card} position={card.position} active={draggedId === card.id} selected={selectedId === card.id} onPointerDown={pointerDown(card.id)} onPointerUp={pointerUp()} onClick={() => setSelectedId(card.id)} /></div>)}</div> : <p className="rounded-xl border border-dashed border-border p-4 text-sm text-muted">Ingen reserver i troppen.</p>}</div>
