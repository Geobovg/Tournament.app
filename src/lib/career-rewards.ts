...
type Reward = { managerBudget: number; key: string };
...
async function award(userId: string, sourceType: string, sourceId: string, reward: Reward): Promise<boolean> {
  ...
  const { data, error } = await db.from("career_reward_events").upsert({ user_id: userId, source_type: sourceType, source_id: sourceId, reward_key: reward.key, manager_budget: reward.managerBudget }, { onConflict: "user_id,source_type,source_id,reward_key", ignoreDuplicates: true }).select("id");
  if (error) throw new Error(error.message);
...
async function awardTournamentResult(userId: string, matchId: string, result: "win" | "draw" | "loss") {
  const rewards: Record<"win" | "draw" | "loss", Reward> = {
    win: { managerBudget: 3, key: "win" },
    draw: { managerBudget: 1, key: "draw" },
    loss: { managerBudget: 0, key: "loss" },
  };
